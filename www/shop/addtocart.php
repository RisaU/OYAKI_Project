...
require_once "dbCon.php";
    ...
    if (isset($_POST['sel_item_id'])) {
        ...
        $get_iteminfo_sql = "SELECT item_title FROM store_items 
        ...
        if (mysqli_num_rows($get_iteminfo_res) < 1) {
            ...
            while ($item_info = mysqli_fetch_array($get_iteminfo_res)) {
                $site_title = stripslashes($item_info['item_title']);
            }

            // free result
            mysqli_free_result($get_iteminfo_res);

            // add info to cart table     store_shoppertrak
            $addtocart_sql = "INSERT INTO store_shoppertrack(session_id, sel_item_id, sel_item_qty,
                            sel_item_size, sel_item_color, date_added) VALUES('" 
                            . $_COOKIE['PHPSESSID'] . "',
                            '" . $safe_sel_item_id . "',
                            '" . $safe_sel_item_qty . "',
                            '" . $safe_sel_item_size . "',
