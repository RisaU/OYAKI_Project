...
END_OF_TEXT;
        ...
        // get size
        $get_sizes_sql = "SELECT item_size FROM store_item_size WHERE item_id=" 
                        . $safe_item_id  . " ORDER BY item_size";
        ...
        if (mysqli_num_rows($get_sizes_res) > 0) {
            ...
            while ($sizes = mysqli_fetch_array($get_sizes_res)) {
                $item_sizes = $sizes['item_size'];
                $display_block .= "<option value=\"" . $item_sizes . "\">"
                                . $item_sizes . "</option>";
            }
        }
        $display_block .= "</select></p>";
